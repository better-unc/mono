...
try {
  ...
  console.error("[Auth Service] Failed to connect to Redis:", error instanceof Error ? error.message : "Unknown error");
  throw error;
}
...
const getApiUrl = (): string => {
  ...
  if (process.env.AUTH_SERVICE_URL) {
    return normalizeUrl(process.env.AUTH_SERVICE_URL);
  }
  ...
  if (process.env.NODE_ENV === "production") {
    throw new Error("BETTER_AUTH_URL, AUTH_SERVICE_URL, API_URL, or RAILWAY_PUBLIC_DOMAIN must be set in production");
  }
...
const getWebUrl = (): string => {
  ...
  if (process.env.WEB_URL) {
    return normalizeUrl(process.env.WEB_URL);
  }

  if (process.env.NODE_ENV === "production") {
    throw new Error("BETTER_AUTH_URL or WEB_URL must be set in production");
  }
...
const getTrustedOrigins = (): string[] => {
  ...
  if (process.env.WEB_URL) {
    origins.push(normalizeUrl(process.env.WEB_URL));
  }
...
console.log(`[Auth Service] Better Auth baseURL: ${apiUrl}`);
